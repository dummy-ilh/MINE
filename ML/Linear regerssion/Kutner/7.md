...
Table 7.1 contains data for a study of the relation of amount of body fat \(Y\) to several possible predictor variables, based on a sample of 20 healthy females 25–34 years old. The predictors are triceps skinfold thickness \(X_1\), thigh circumference \(X_2\), and midarm circumference \(X_3\). The amount of body fat \(Y\) was obtained by an expensive procedure (immersion in water), so a regression model using these predictors would be useful for estimating body fat from easily obtained measurements.

Table 7.2 summarizes regression results for four fitted models:
...
To keep track of the model, the notation is modified as follows. The regression sum of squares when only \(X_1\) is in the model is denoted
\[
\text{SSR}(X_1).
\]
From Table 7.2(a),
...
\text{SSR}(X_1) = 352.27.
\]
The error sum of squares for this model is denoted
\[
\text{SSE}(X_1),
\]
and from Table 7.2(a),
\[
\text{SSE}(X_1) = 143.12.
\]

Similarly, when \(X_1\) and \(X_2\) are in the model, Table 7.2(c) gives
\[
\text{SSR}(X_1, X_2) = 385.44, \quad \text{SSE}(X_1, X_2) = 109.95.
\]

Note that
\[
\text{SSE}(X_1, X_2) = 109.95 < \text{SSE}(X_1) = 143.12.
\]
The reduction in the error sum of squares caused by adding \(X_2\) to a model that already has \(X_1\) is called an *extra sum of squares* and is denoted
\[
\text{SSR}(X_2 \mid X_1).
\]
It is defined by
\[
\text{SSR}(X_2 \mid X_1) = \text{SSE}(X_1) - \text{SSE}(X_1, X_2)
= 143.12 - 109.95 = 33.17.
\]

Equivalently, the extra sum of squares can be written in terms of regression sums of squares:
\[
\text{SSR}(X_2 \mid X_1) = \text{SSR}(X_1, X_2) - \text{SSR}(X_1)
= 385.44 - 352.27 = 33.17.
\]

This equivalence follows from the basic ANOVA identity
\[
\text{SSTO} = \text{SSR} + \text{SSE},
\]
since \(\text{SSTO}\) does not depend on which regression model is fitted.

---

### Extra sums of squares: definitions

The extra sum of squares \(\text{SSR}(X_2 \mid X_1)\) measures the marginal effect of adding \(X_2\) to the regression model when \(X_1\) is already included. More generally:

- For adding a single variable \(X_j\) to a model containing a set of predictors \(X_A\),
  \[
  \text{SSR}(X_j \mid X_A) = \text{SSE}(X_A) - \text{SSE}(X_A, X_j)
  \]
  or equivalently
  \[
  \text{SSR}(X_j \mid X_A) = \text{SSR}(X_A, X_j) - \text{SSR}(X_A).
  \]

- For adding several variables \(X_B\) to a model containing \(X_A\),
  \[
  \text{SSR}(X_B \mid X_A) = \text{SSE}(X_A) - \text{SSE}(X_A, X_B)
  \]
  or
  \[
  \text{SSR}(X_B \mid X_A) = \text{SSR}(X_A, X_B) - \text{SSR}(X_A).
  \]
...
From Tables 7.2(c) and 7.2(d), the marginal effect of adding \(X_3\) to the model already containing \(X_1\) and \(X_2\) is
\[
\text{SSR}(X_3 \mid X_1, X_2)
...
### Example: adding \(X_2\) and \(X_3\) together

For the marginal effect of adding both \(X_2\) and \(X_3\) to the model already containing \(X_1\),
\[
\text{SSR}(X_2, X_3 \mid X_1)
= \text{SSE}(X_1) - \text{SSE}(X_1, X_2, X_3)
= 143.12 - 98.41 = 44.71,
\]
or equivalently,
\[
