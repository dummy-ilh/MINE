...
**Properties of Residuals** — In simple linear regression, residuals $e_i = Y_i - \hat{Y}_i$ have several important theoretical properties that are central to model diagnostics. First, the **mean of the residuals** is always zero, that is $\bar e = \frac{1}{n}\sum_{i=1}^n e_i = 0$, which is a direct consequence of the least squares normal equations; therefore, this property does not provide any evidence about whether the true errors $\varepsilon_i$ satisfy $E(\varepsilon_i)=0$, since it is enforced by the fitting procedure itself. Second, the **variance of the residuals** is estimated by  
$$
s^2 = \frac{\sum_{i=1}^n (e_i - \bar e)^2}{n-2} = \frac{\sum_{i=1}^n e_i^2}{n-2} = \frac{\text{SSE}}{n-2} = \text{MSE},
$$
where the denominator $n-2$ reflects the loss of two degrees of freedom due to estimation of the intercept and slope; if the regression model is correct, $\text{MSE}$ is an unbiased estimator of the true error variance $\sigma^2$. Finally, the **residuals are not independent**, because each fitted value $\hat Y_i$ depends on all observations through the estimated regression line; as a result, the residuals satisfy two constraints, $\sum_{i=1}^n e_i = 0$ and $\sum_{i=1}^n X_i e_i = 0$. Although this dependence is theoretically important, it becomes negligible in practice when the sample size $n$ is large relative to the number of parameters in the model, allowing residual-based diagnostics to be used effectively.
...
At times, it is helpful to **standardize residuals** when conducting residual analysis in regression. Since the standard deviation of the true error terms $\varepsilon_i$ is $\sigma$, which is estimated by $\sqrt{\text{MSE}}$, a natural form of standardization is  
$$
e_i^* = \frac{e_i - \bar e}{\sqrt{\text{MSE}}} = \frac{e_i}{\sqrt{\text{MSE}},}

$$
where the simplification follows because the mean of the residuals satisfies $\bar e = 0$. If $\sqrt{\text{MSE}}$ were the true standard deviation of the residual $e_i$, then $e_i^*$ would be called a *studentized residual*. However, the actual standard deviation of $e_i$ depends on the leverage of the observation and therefore varies across residuals, making $\sqrt{\text{MSE}}$ only an approximation. For this reason, the quantity $e_i^*$ in (3.5) is referred to as a **semistudentized residual** rather than a fully studentized one. Despite this approximation, semistudentized residuals are extremely useful in practice, particularly for detecting outlying observations, and their fully studentized counterparts are examined later (in Chapter 10).
...
7. Normal probability plot of residual
8. 
...
## 3.5 Correlation Test for Nonnormality 
...
### Lowess Method (Locally Weighted Scatterplot Smoothing) 
