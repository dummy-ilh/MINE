...
When the effect of XI on the mean response does not depend on the level of X 2 , and correspondingly the effect of X 2 does not depend on the level of X I, the two predictor variables are said to have additive effects or not to interact. Thus, the first-order regression  model (6.1) is designed for predictor variables whose effects on the mean response are additive or do not interact. The parameters f31 and f32 are sometimes called partial regression coefficients because they reflect the partial effect of one predictor variable when the other predictor variable is included in the model and is held constant
...
he first-order regression model then is as follows:
Yi = f30 + f3I X iI + f32 X i2 + £i
where:
Xi1 = patient's age
X- _ { 1 if patient female
i2 -   0 if patient male
