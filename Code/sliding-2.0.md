...
def max_subarray_sum_size_k(nums, k):
    # build the first window up front
    current_sum = 0
    for i in range(k):
        current_sum += nums[i]
    ...
    # now just slide: drop the trailing element, add the leading element
...
Notice there's no `while` and no "is the window full yet?" check — because the first window is built before the loop even starts, the loop body only ever has to do one job: slide by one.

**Naive version first (so the optimization is obvious):**

```python
def max_subarray_sum_size_k_naive(nums, k):
    max_sum = float('-inf')
...
The naive version re-sums all `k` elements at every position — `O(n·k)`. The optimized version above removes one element and adds one element per step — `O(n)`.

**Dry run:** Already done above in Section 1 (array `[2,1,5,1,3,2]`, `k=3` → answer `9`). With this template, the trace looks like:

```
Build first window: current_sum = 2+1+5 = 8, max_sum = 8
i=0: current_sum -= nums[0]=2 → 6; current_sum += nums[3]=1 → 7   max_sum stays 8
i=1: current_sum -= nums[1]=1 → 6; current_sum += nums[4]=3 → 9   max_sum = 9
i=2: current_sum -= nums[2]=5 → 4; current_sum += nums[5]=2 → 6   max_sum stays 9
...
Same as the base template, just divide `max_sum` by `k` at the end.
```python
return max_sum / k
```

**LC 1456 — Max Vowels in a Substring of Length K**
Instead of summing numbers, add `1` for each vowel in the first window, then slide the same way.
```python
current_count = 0
for i in range(k):
    if nums[i] in 'aeiou':
...
for i in range(len(nums) - k):
    if nums[i] in 'aeiou':
        current_count -= 1       # trailing char leaves
    if nums[i + k] in 'aeiou':
        current_count += 1       # leading char enters
...
Add a `Counter` alongside `current_sum`. Only count a window as valid if all `k` elements are distinct (`len(window_counter) == k`).
```python
...
for i in range(len(nums) - k):
    ...
    current_sum -= trailing_value
    current_sum += leading_value
...
No sum needed — build a set from the first `k+1` elements, then check membership before sliding.
```python
window_set = set(nums[:k + 1])
if len(window_set) < k + 1:
    return True   # first window already had a duplicate

for i in range(len(nums) - k - 1):
    trailing_value = nums[i]
    leading_value = nums[i + k + 1]

    window_set.discard(trailing_value)
    if leading_value in window_set:
        return True
    window_set.add(leading_value)

return False
```

**LC 2 (Bonus) — Max Subarray Product (Size K)**
Same idea as the sum version, but with multiplication and division instead of addition and subtraction:
```python
def max_subarray_product_size_k(nums, k):
    ...
    for i in range(len(nums) - k):
        current_product /= nums[i]          # trailing element leaves
        current_product *= nums[i + k]      # leading element enters
...
**LC 2 (Bonus) — Count Subarrays of Size K With a Target Sum**
Same fixed-window slide, but instead of tracking a max, you just count how many windows hit the target exactly:
```python
...
**Related variant — Find Subarray With Sum Exactly Equal to Target**

Slightly different question: instead of "smallest window with sum `>= target`", this asks "find any one window whose sum is `exactly` target" (assuming positive numbers, so growing the window only increases the sum — never overshoots back down). The shrink condition changes from `while window_sum >= target` to `while window_sum > target`, and you check for an exact match right after shrinking:
...
Same skeleton as the base template (expand on `end`, shrink while "too big"), just a different stopping condition and a different thing being recorded (indices instead of a length).
...
**Mental model:** Start with the simplest possible version, then build up.

**Step 1 — Simple version (unique characters only):**

If you can assume no character repeats inside the pattern, you don't even need counts — a `set` comparison is enough.
...
def has_substring_anagram(s, anagram):
    ...
    for i in range(len(s) - k):
        ...
        window_set.remove(trailing_char)
        window_set.add(leading_char)
...
This is `O(n·k)` because comparing two sets takes up to `O(k)`, but it's the clearest way to see the "slide and compare" idea before adding frequency counts.

**Step 2 — Real version (duplicates allowed, using `Counter`):**

Most anagram problems allow repeated letters (e.g. `"aab"` vs `"aba"`), so you need counts, not just membership. `Counter` from `collections` handles this naturally — two `Counter`s are equal only if every character has the exact same count in both.
...
Notice this is the *exact same slide pattern* as Template 1 (`trailing_char` leaves, `leading_char` enters) — only now the "value" being added and removed is a character's count inside a `Counter`, instead of a number inside a running sum. Same skeleton, different payload.

**Why this is `O(n·k)` and not `O(n)`:** comparing two `Counter` objects (`anagram_counter == window_counter`) walks through up to `k` keys every single iteration. The `matched`-counter trick shown later in this template removes that comparison entirely, bringing it down to `O(n)` — but the version above is the one to learn first, because it's the one that's "easy" to read and reason about.

**Step 3 — Optimized version (O(n), using a `matched` counter):**

The Counter-equality check above is correct but slightly slow. Here's the upgrade: instead of comparing two `Counter`s every step, track a single integer `matched` = how many distinct characters currently have the exact required count in the window.
...
# SECTION 3.5 — Foundational Worked Examples (Quick Reference)

These seven examples are the ones to internalize first — they're written in the plainest possible style (build the first window, then slide) and every template above is just a variation on one of these.

**1. Max Subarray Sum, Size K — Naive**
```python
def max_subarray_sum_size_k(nums, k):
    max_sum = float('-inf')
    for i in range(len(nums) - k + 1):
    ...
    return max_sum
```
Time: `O(n·k)` · Space: `O(1)` (or `O(k)` depending on slice implementation)

**2. Max Subarray Sum, Size K — Sliding Window**
```python
def max_subarray_sum_size_k(nums, k):
    current_sum = 0
    for i in range(k):
    ...
    return max_sum
```
Time: `O(n)` · Space: `O(1)` → this is **Template 1**.

**3. Max Subarray Product, Size K**
```python
def max_subarray_product_size_k(nums, k):
    current_product = 1
    for i in range(k):
    ...
    return max_product
```
Time: `O(n)` · Space: `O(1)` → **Template 1**, multiplication instead of addition.

**4. Subarray Target Sum, Size K (count matches)**
```python
def subarray_target_sum_size_k(nums, target, k):
    current_sum = 0
    for i in range(k):
    ...
    return count
```
Time: `O(n)` · Space: `O(1)` → **Template 1**, counting instead of tracking a max.

**5. Has Substring Anagram (unique characters)**
```python
def has_substring_anagram(s, anagram):
    k = len(anagram)
    window_set = set(s[:k])
    ...
    return False
```
Time: `O(n·k)` · Space: `O(k)` → simplified entry point into **Template 6**.

**6. Count Substring Anagrams (duplicates allowed)**
```python
from collections import Counter
...
def count_substring_anagrams(s, anagram):
    ...
    return num_matches
```
Time: `O(n·k)` · Space: `O(k)` → **Template 6**, full version.

**7. Find Subarray With Sum Exactly Equal to Target (variable window)**
```python
def find_subarray_sum(nums, target):
    start = 0
    current_sum = 0
    ...
    return None
```
Time: `O(n)` · Space: `O(1)` → your first taste of a **variable-size** window (Templates 2 & 3's shared skeleton: expand, shrink while too big, check).
